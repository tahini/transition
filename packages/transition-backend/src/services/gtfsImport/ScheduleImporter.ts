...
const generateSchedulesForLine = async (
    ...
    try {
        ...
        for (let scheduleGroupIdx = 0; scheduleGroupIdx < groupedSchedulesForLine.length; scheduleGroupIdx++) {
            ...
            if (existingSchedules[schedule.attributes.service_id] && typeof existingSchedules[schedule.attributes.service_id].attributes.integer_id === 'number') {
                // Delete previous schedules for this service
                await scheduleDbQueries.delete(existingSchedules[schedule.attributes.service_id].attributes.integer_id as number);
            }
        }

...
const schedulePeriodsAttributesParser = function (attributes: any): SchedulePeriod {
    const { id, uuid, period_start_at_seconds, period_end_at_seconds, custom_start_at_seconds, custom_end_at_seconds, ...rest } =
        attributes;
    return {
        ...rest,
        id: uuid,
...
const scheduleTripsAttributesCleaner = function (attributes: Partial<SchedulePeriodTrip>): { [key: string]: any } {
    ...
    _attributes.id = typeof attributes.integer_id === 'number' && attributes.integer_id > 0 ? attributes.integer_id : undefined;
    return _attributes;
};
...
const save = async function (scheduleData: ScheduleAttributes, options: { transaction?: Knex.Transaction } = {}) {
    const scheduleExists = scheduleData.integer_id ? await exists(knex, scheduleTable, scheduleData.integer_id, { transaction: options.transaction }) : false;
...
const readScheduleData = async function (id: number, options: { transaction?: Knex.Transaction } = {}) {
    const schedule = scheduleAttributesParser(await read(knex, scheduleTable, undefined, '*', id, options)) as ScheduleAttributes;
...
const deleteScheduleData = async function (id: number, options: Parameters<typeof deleteRecord>[3] = {}) {

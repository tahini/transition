...
export async function up(knex: Knex): Promise<unknown> {
    // Rename the current uuid columns, this will keep the foreign key relationships
    ...
    // Add the new auto-incrementing id columns and columns for foreign key with nullable values
    await knex.schema.alterTable(scheduleTblName, (table: Knex.TableBuilder) => {
    ...
    // Change the primary key columns and drop the foreign uuid key columns, make the new foreign key columns not nullable
    await knex.schema.alterTable(scheduleTripTblName, (table: Knex.TableBuilder) => {

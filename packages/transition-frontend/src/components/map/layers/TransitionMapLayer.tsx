...
const addEvents = (
    ...
    if (onDragStartEvt || onDragEvt || onDragEndEvt) {
        if (onDragStartEvt) {
            layerEvents.onDragStart = (info: PickingInfo, e: MjolnirGestureEvent) => {
                console.log('in layers on drag start', props.layerDescription.id);
                props.setDragging(true);
                onDragStartEvt.filter(checkHandler).forEach((ev) => ev.handler(info, e, props.mapCallbacks));
            };
        } else {
            layerEvents.onDragStart = (info: PickingInfo, e: MjolnirGestureEvent) => {
                console.log('in layers on drag start', props.layerDescription.id);
    ...
    if (onDragEvt) {
        layerEvents.onDrag = (info: PickingInfo, e: MjolnirGestureEvent) => {
            console.log('in layers on drag', props.layerDescription.id);
    ...
    if (onDragStartEvt || onDragEvt || onDragEndEvt) {
        if (onDragEndEvt) {
            layerEvents.onDragEnd = (info: PickingInfo, e: MjolnirGestureEvent) => {
                console.log('in layers on drag end', props.layerDescription.id);
                props.setDragging(false);
                onDragEndEvt.filter(checkHandler).forEach((ev) => ev.handler(info, e, props.mapCallbacks));
            };
        } else {
            layerEvents.onDragEnd = (info: PickingInfo, e: MjolnirGestureEvent) => {
                console.log('in layers on drag end', props.layerDescription.id);
